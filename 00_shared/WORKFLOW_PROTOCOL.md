...
5. 默认逐问推进：只有当前问题完成模型确认后，下一问才进入方案生成和审批。
...
9. 每完成一个问题的模型确认和图表审批后，Codex 必须写入该问题对应 LaTeX 小节并立即编译一次 PDF。
10. 摘要、整体问题分析、模型检验、模型评价与推广等总结性内容必须等全部问题完成后再填写。
...
12. Codex 生成最终论文图表并写入 LaTeX。
13. 用户审批图表。
14. Codex 写入论文正文和代码附录。
15. Codex 编译 PDF 并运行版面检查，确认图表页码、图文顺序和大空白问题。
16. 当前问题模型确认后，下一问才解锁。
